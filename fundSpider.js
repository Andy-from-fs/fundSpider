...
class ConcurrentCtrl {
  ...
  constructor(parent, splitNum, fn, dataArray = [], collection) {
    ...
    this.restNum =
      this.length % splitNum === 0 ? splitNum : this.length % splitNum; // 最后一次分段的余下次数
  ...
  fundToSave(error, codesArray = []) {
    if (!error) {
      ...
      MongoClient.connect(this.dbUrl, (err, db) => {
        ...
        // 并发控制器实例
        let concurrentCtrl = new ConcurrentCtrl(
          this,
          this.fragmentSize,
          this.fundFragmentSave,
          codesArray,
          dbCollection
        );
        // 事件监听
  ...
  fundSave(_codesArray) {
    if (!_codesArray) {
      ...
      if (_codesArray.length > 0) {
        ...
        return 'not enough codes to fetch';
      }
    }
  }
